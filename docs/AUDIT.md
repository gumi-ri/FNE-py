...
- 验证基线：90 项自动化测试 + 真机样本（4 个 `.ncm` 46–66 MB、10 个 `.mflac` / `.mgg`）
  + 全新虚拟环境 wheel 安装验证
...
| 项目 | 结果 |
| --- | --- |
| 自动化测试 | 90 项：Python 3.14 + tkinter 下 88 通过 / 2 跳过；无 tkinter 的 3.13 下 84 通过 / 6 跳过 |
| pyflakes | 零告警 |
| 真机 `.ncm`（4 个，46–66 MB） | 4 / 4 成功，音频帧与 Go 版**逐字节一致** |
| 真机 `.mflac` / `.mgg`（10 个） | 10 / 10 成功，9 / 9 FLAC 音频帧**逐字节一致** |
...
审计建议的「打包验证」已实际执行，这也是本轮唯一新增的验证环节。
...
- **CI 未配置**：`pytest` + `pyflakes` 适合进 CI，但真机部分（需 QQ 音乐客户端与授权样本）
  只能留在发布前手工清单里
